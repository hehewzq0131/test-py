...
def create_report(testcase_dir,test_report_dir):
    """执行测试用例，生成新的报告"""
    discover=unittest.defaultTestLoader.discover(testcase_dir, pattern='Test_*.py') # 获取测试集
    ...
    runner = HTMLTestRunner(stream=fp,title=u'测试报告',description=u'用例执行情况：')
      #  需要指定文件对象和标题, 需要指定文件对象和标题
    runner.run(discover)   #  执行测试集
...
def find_newfile(test_dir):
    ...
    print('eeeeeeeeee%s:, %s'% (file_path,lists[-1]))
    print('333',file_path.split("\\")[-1])
    return file_path
...
def send_email(filepath):
    ...
    # 发送邮箱用户名/密码
    user = '369571711@qq.com'
    # 修改成你自己邮箱的授权码
    password = 'wwwwwwwww'
    # 发送邮箱
    sender = 'smtp.exmail.qq.com'
    # 多个接收邮箱，单个收件人的话，直接是receiver='XXX@126.com'
    receiver = ['zeqing.wang@taojiji.com', '369571711@qq.com']
    # 发送邮件主题
...
if(__name__=="__main__"):
    #D:\work\python\autoSendEmail-master\autoSendEmail-master
    testcase_dir=r'D:\study\python\autoSendEmail-master\autoSendEmail-master\test_case'
    test_report_dir=r'D:\study\python\autoSendEmail-master\autoSendEmail-master\testReport'
